	...
	_data(data)
{
	...
	sprite.setOrigin(8, 32);
	sprite.setPosition(pos);	
	sprite.setTextureRect(sf::IntRect(0, 0, 16, 64));


	hitbox.setSize(sf::Vector2f(16, 64));
	hitbox.setOrigin(8, 32);
...
void RopeSegment::InitPhysics()
{
	...
	b2BodyDef bodyDef;
	bodyDef.position = b2Vec2((sprite.getPosition().x + collisionBox.left) / SCALE, (sprite.getPosition().y - collisionBox.height / 2) / SCALE);
	...
	testPosition.y -= (collisionBox.height / 2) / SCALE;
